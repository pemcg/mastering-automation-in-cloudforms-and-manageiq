...
[TIP]
====
Don't try searching the CloudForms/ManageIQ sources for +def find_by_description+, these are not statically defined methods and so don't exist in the code. In a future version of CloudForms/ManageIQ they will be deprecated  in favour of a more current Rails-like syntax using +where+, for example:
[source,ruby]
----
...
The following list shows the class definition for some of the CloudForms/ManageIQ _Botvinnik_ MiqAeService model classes, showing their immediate superclass:
