[[running_an_ansible_tower_job_from_a_button]]
...
CloudForms 4.2/ManageIQ _Euwe_ now requires the *Prompt on launch* checkbox to be ticked for the *Limit* element (see <<c27ci5>>). When we submit a job request from CloudForms/ManageIQ the limit switch is used to pass the name of the host or comma-separated list of hosts on which to run the job.

[[c27ci5]]
.Setting Prompt on launch for the Limit element
image::images/ch27c_ss11.png[Screenshot,250,align="center"]
{zwsp} +
...
We must also ensure that *Prompt on Launch* is checked, to allow the variables to be overridden from CloudForms/ManageIQ if we wish (see <<c27ci6>>).
...
For our example we select the *Simple LAMP Stack* job template, then select *Configuration -> Create Service Dialog from this Job Template* (see <<c27ci7>>).
...
Having created the dialog, we can add a button to our VM button group if we wish. Our button will use the new "Simple LAMP Stack" dialog, and will call the __ansible_tower_job instance__, as shown in <<c27ci9>>.
...
We're ready to test the operation of the Ansible playbook, called by CloudForms/ManageIQ. We'll select a suitable CentOS 7 VM that has been configured with the ansible-remote user and ssh key, and order from the *Simple LAMP Stack* button (see <<c27ci10>>).

[[c27ci10]]
.Adding the button
image::images/ch27c_ss10.png[Screenshot,400,align="center"]
{zwsp} +

We fill in the dialog parameters; in this case we'll override the default values for *ntpserver* and *dbname* (see <<c27ci11>>).
