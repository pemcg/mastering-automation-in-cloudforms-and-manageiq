...
If the non-existant URI __/ConfigurationManagement/AnsibleTower/Operations/JobTemplate/configure_database_server__ was called, the _.missing_ instance would be run with the *job_template_name* attribute given the value of *configure_database_server*.

=== Summary

This chapter completes our coverage of the objects that we work with in the Automate Datastore. The schema and instance features that we've learnt about here are used less frequently, but are still very useful tools to have in our scripting toolbag.
