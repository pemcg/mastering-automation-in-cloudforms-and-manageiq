...
As an example the *compute.instance.power_on.end* OpenStack event was not handled by default with CloudForms 4.0/ManageIQ _Capablanca_ (this has been fixed with CloudForms 4.1/ManageIQ _Darga_). If we looked in _evm.log_ we would see:
...
There was already a _ManageIQ/System/Event/EmsEvent/OpenStack/compute.instance.power_off.end_ instance to handle the *compute.instance.power_off.end* event. This instance calls two event_handlers (see <<c15i11>>).
